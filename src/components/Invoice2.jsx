            ...
            <div ref={divRef} className='bg-white px-10 py-10 w-[50vw]'>
                <div className='mb-10'>
                    <div className='flex justify-between items-center'>
                        <div className=''>
                            <p className='font-semibold text-xl text-[#0070C0]'>{data?.invoice_type?.toUpperCase()}</p>
                            <p className='font-semibold text-md'>{data?.bill_to_name}</p>
                            <p className='font-semibold text-md'>GSTIN <span className='font-medium'>{data?.bill_to_gst_in}</span></p>
                            <p className='font-medium text-sm'>{data?.bill_to_address}</p>
                            <p className='font-medium text-sm'>{data?.bill_to_city}, {data?.bill_to_state}, {data?.bill_to_country} - {data?.bill_to_pincode}</p>
                        </div>
                        <div>
                            {data?.bill_to_logo ? (
                    ...
                    <div className='grid grid-cols-[1fr,1fr,1fr] mt-10 text-xs gap-5'>
                        <div>
                            <p className=''><span className='font-semibold'>Seller Details:</span> {data?.seller_to_name}</p>
                            <p>{data?.seller_to_address}, {data?.seller_to_city}, {data?.seller_to_state}, {data?.shipped_to_country} - {data?.seller_to_pincode}</p>
                            <p>GSTIN: {data?.seller_to_gst_in}</p>
                            <p>Phone No. {data?.seller_to_phone}</p>
                        </div>
                        <div className=''>
                            <p className=''><span className='font-semibold'>Details of Buyer | Shipped to: </span> {data?.shipped_to_name}</p>
                            <p>{data?.shipped_to_address}, {data?.shipped_to_city}, {data?.shipped_to_state} </p>
                            <p>{data?.shipped_to_country} - {data?.shipped_to_pincode}</p>
                            <p>GSTIN: {data?.shipped_to_gst_in}</p>
                            <p className='mb-5'>Phone No. {data?.shipped_to_phone}</p>
                        </div>
                        <div className=''>
                            {data?.invoice_no && (
                            ...
                            {data?.eway_no && (
                                <p className=''><span className='font-semibold'>Eway No. :</span> {data?.eway_no}</p>
                            )}
                            {data?.delivery_time && (
                ...
                <div className='grid grid-cols-[2.6fr,1fr] mt-4 text-xs'>
                    <div></div>
                    <div>
                ...
                <div className='flex justify-between border-b-[3px] border-blue-400 mt-5 pb-2 text-xs'>
                    <div>
                        <p className=''>Total Items / Qty : {totalQty} {data?.product_details?.[0]?.qty_type}</p>
                    </div>
                    <div className='w-[75%]'>
                        <p className='font-medium'>Total amount (in words): {numberToWords(totals?.totalAmount?.toFixed()).toUpperCase()}</p>
                    </div>
                </div>
                ...
                <div className='grid grid-cols-[2fr,1fr] mt-10 text-xs'>
                    ...
                    <div className='flex flex-col items-center'>
                        ...
                        <img src={data.upload_sign} alt="" className='h-[20px] mt-10' />
                    </div>
                </div>
