...
const Addchemical = () => {
  ...
  const [suggestedChemicalLoading, setSuggestedChemicalLoading] =
    useState(false);
  ...
  // NEW STATE FOR SALE/BUY DROPDOWN
  const [listingType, setListingType] = useState(""); // "sale" or "buy"
  ...
  //     } else {
  //       try {
  //         const response = await axios.post(`${BASE_URL}api/catalog`, formData, {
  //           headers: {
  //             "Content-Type": "multipart/form-data",
  //             Authorization: token,
  //           },
  ...
  const openModal = async () => {
    ...
    if (listingType === "sale" || listingType === "buy") {
      ...
      formData.append("product_id", selectedCatalog._id);
      formData.append("listing_type", listingType); // Add listing type
      formData.append("active_chemicals", activeChemicalStatus);
      formData.append("status", "active");

      // Add other fields if they have values (optional for sale/buy)
      ...
      if (qValue) formData.append("qty", qValue);
      if (qtyType) formData.append("qty_type", qtyType);
      if (origin) formData.append("country_origin", origin);
      if (hsnCode) formData.append("hsn_code", hsnCode);
      if (capacity) formData.append("supply_capacity", capacity);
      if (purity) formData.append("purity", purity);
      if (value) formData.append("one_lot_qty", value);
      if (type) formData.append("one_lot_qty_type", type);
      if (samplePrice) formData.append("one_lot_qty_price", samplePrice);
      if (qNumber) formData.append("max_lot_qty", qNumber);
      if (samplePrice) formData.append("sample_price", samplePrice);
      if (synonyms) formData.append("synonyms", synonyms);
      let coa;
      if (productId && pdfFile) {
        coa = document;
      } else if (productId) {
    ...
    // ORIGINAL VALIDATION LOGIC (only when listingType is not selected)
    if (
      !category &&
      !document &&
      !origin &&
  ...
  const fetchCatelogData = async () => {
    ...
    const res = await fetch(
      `${BASE_URL}api/product/displayAllProductWithoutToken`,
      {
        method: "GET",
        headers: {
  ...
  useEffect(() => {
    const selectedCatalog =
      catelogs && catelogs.find((item) => item._id === productId?.product_id);
    ...
    setSynonyms(productId?.synonyms || "");
     setListingType(productId?.listing_type || "");
  }, [productId, catelogs]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      await axios
        .post(`${BASE_URL}api/productByCompany/insert`, formDataToSend, {
          headers: {
            Authorization: token,
            "Content-Type": "multipart/form-data",
          },
        ...
        .catch((error) => {
          toast.error(
            error?.response?.data?.message || "Failed to add chemical",
            {
              position: toast.POSITION.BOTTOM_RIGHT,
              autoClose: 1000,
            }
                ...
                {catelogs?.length > 1 ? (
                  filteredCatalogs?.map((catalog) => (
                    <div
                      ...
                      className="py-2 ps-3 hover:bg-gray-200 rounded-lg font-medium"
                    >
                      {catalog.name_of_chemical}
                    </div>
                  ))
                ...
                <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-3xl">
                  ...
                  <div class="bg-gray-100 px-4 py-5 sm:px-6">
                    <div>
                      <div className="grid grid-cols-[1fr,1fr,1fr] gap-4 mb-6">
                        <div>
                          ...
                          <input
                            ...
                            type="text"
                            className="border border-[#0A122A]/[.6] rounded-[4px] py-1 px-3 w-full outline-none bg-white/[0.2]"
                          />
                        </div>
                        <div>
                          <p className="mb-1 text-sm font-medium">
                      ...
                      <div className="grid grid-cols-[1fr,1fr,1fr] gap-4 mb-6">
                        <div>
                          <p className="mb-1 text-sm font-medium">
                            CAS No. <span className="text-red-600">*</span>
                          </p>
                          <input
                            onInput={(e) =>
                            (e.target.value = e.target.value.replace(
                      ...
                      <div className="grid grid-cols-[1fr,1fr,1fr] gap-4 mb-6">
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={handleChange}
                            type="file"
                            className="file:bg-black file:text-white file:rounded border border-[#0A122A]/[.6] rounded-[4px] py-1 px-3 w-full outline-none"
                          />
                        </div>
                      </div>
                    </div>
                  ...
                  <div class="bg-white flex justify-end mx-8 py-3">
                    <button
                      ...
                      onClick={() => setIsAddChemical(false)}
                    >
                      Cancel
                    </button>
                    {suggestedChemicalLoading ? (
                      <div className="bg-[#0A122A] rounded-md ml-4">
                        <img
                          src={loader}
                          alt=""
                          className="h-[40px] px-4 animate-spin py-2"
                        />
                      </div>
                    ) : (
                      <button
            ...
            <div className="mt-10">
              <div className="flex bg-white gap-6 border py-4 px-4 rounded-lg shadow-md">
                <div className="border h-100% flex items-center rounded-md">
                  <div
                    className="w-[350px] h-[350px] bg-cover bg-center bg-no-repeat flex items-center justify-center"
                    style={{ backgroundImage: `url(${chemicalLogo})` }}
                  >
                    <img
                      src={selectedCatalog?.structure}
                      alt=""
                ...
                <div className="w-full">
                  <h2 className="text-3xl font-semibold w-full pb-3">
                    {selectedCatalog?.name_of_chemical}
                  </h2>
                  <hr />
                  <div className="py-3">
                ...
                <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-md py-4">
                  <div class="bg-white py-3">
                    <div class="flex flex-col items-center">
                      ...
                      <p className="text-sm">
                        Your catalog has been updated successfully
                      </p>
                    </div>
                  </div>
                ...
                <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-md py-4">
                  <div class="bg-white py-3">
                    <div class="flex flex-col items-center">
                      <img src={right} alt="" className="h-[80px]" />
                      <h3 className="font-semibold text-2xl mt-4">
                        Chemical Submitted Successfully
                      </h3>
