...
import ReturnModal from "./ReturnInquiryPage";
...
const getStatusColorClass = (status) => {
  const statusColors = {
    ...
    // Success/completion statuses - Green
    refund_successful: "bg-green-500 text-green-800",
    refund_processed: "bg-green-100 text-green-800",
    credit_note_issued: "bg-green-100 text-green-800",
    debit_note_issued: "bg-green-100 text-green-800",
    amount_adjusted: "bg-green-100 text-green-800",
    product_replaced: "bg-green-100 text-green-800",
    quality_issue_resolved: "bg-green-100 text-green-800",
    payment_settled: "bg-green-100 text-green-800",
    partial_refund_issued: "bg-green-100 text-green-800",
    full_refund_issued: "bg-green-100 text-green-800",
...
const Buying_inquiry_detail = () => {
  ...
  console.log(alreadyData, "already");
  ...
  const [currentReturnOrderId, setCurrentReturnOrderId] = useState(null); // set this when a return exists
  ...
  useEffect(() => {
    socket?.on("statusChange", (newMessage) => {
      ...
      setStatusArray(
        newMessage?.inquiry_status_value.map((item) => item.status)
      );
    });
  ...
  const fetchReturnStatusHistory = async (returnOrderId) => {
    try {
      const token = `Bearer ${localStorage.getItem("chemicalToken")}`;
      const response = await axios.get(
        `${BASE_URL}api/return_order/status-history/${returnOrderId}`,
        {
          headers: {
            Authorization: token,
          },
      ...
      if (response.status === 200) {
        setReturnStatusHistory(
          response.data.data[0]?.return_status_value || []
        );
        setSelectedReturnRequest(returnOrderId);
      }
    } catch (error) {
      console.error("Error fetching return status history:", error);
    }
  };
  ...
  const getReturnStatusImage = (status) => {
    const statusImages = {
      pending: exclamation, // or create specific return status images
      under_review: underReviewLogo,
      approved: right,
      rejected: rejectedLogo,
      in_transit: intransitLogo,
      received: deleveredLogo,
      refund_successful: right, // or create a refund success image
    };
    return statusImages[status] || exclamation;
  };
  ...
  const fetchReturnRequests = async () => {
    try {
      const token = `Bearer ${localStorage.getItem("chemicalToken")}`;
      const response = await axios.get(
        `${BASE_URL}api/return_order/getReturnRequests?inquiry_id=${_id}`,
        {
          headers: {
            Authorization: token,
          },
  ...
  const updateReturnStatus = async (returnId, newStatus) => {
    setLoadingReturnStatus(true);
    try {
      const token = `Bearer ${localStorage.getItem("chemicalToken")}`;
      const response = await axios.post(
        `${BASE_URL}api/return_order/insert-status`,
        {
      ...
      setLoadingReturnStatus(false);
    }
  };
  // Call this function when needed, for example:
  useEffect(() => {
    if (_id && inquiryStatus === "delivered") {
      fetchReturnRequests();
    }
  }, [_id, inquiryStatus, userPost]);

  const fetchInquiryProgresStatus = async () => {
    const token = `Bearer ${localStorage.getItem("chemicalToken")}`;

    const res = await fetch(
      `${BASE_URL}api/inquiry_status/inquiry_status_display/${_id}`,
      {
        method: "GET",
        headers: {
    ...
    const hasUnderReview = value?.some(
      (visit) => visit.status === "Under Review"
    );
  ...
  const SellingInquiryList = async () => {
    const token = `Bearer ${localStorage.getItem("chemicalToken")}`;
    const res = await fetch(
      `${BASE_URL}api/inquiryRoutes/inquiryDetailsForCompany/${_id}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
  ...
  const [currentStatusIndexForSeller, setCurrentStatusIndexForSeller] =
    useState(-1);
  const [currentStatusIndexForBuyer, setCurrentStatusIndexForBuyer] =
    useState(-1);
  ...
  const percentCompleteForSeller =
    currentStatusIndexForSeller >= 0
      ? ((currentStatusIndexForSeller + 0.5) / statuses.length) * 100
      : 0;

  // Calculate the percentage of completion for the seller
  const percentCompleteForBuyer =
    currentStatusIndexForBuyer >= 0
  ...
  const prefixSubmit = () => {
    ...
    navigate(`/company/generate-new-inquiry-po/${_id}`, {
      state: { abc: "PO", prefix: finalPrefix },
    });
  };
  ...
  const handleInvoicePrefix = () => {
    ...
    navigate(`/company/generate-inquiry-invoice/${_id}`, {
      state: { abc: "Invoice", prefix: finalPrefix },
    });
  };
  ...
  const fetchPaymentHistory = async () => {
    ...
    const res = await fetch(
      `${BASE_URL}api/payment/displayList?inquiry_id=${_id}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
  ...
  const handleReturnOrder = async () => {
    ...
    if (
      returnQty > inquiryQty &&
      alreadyData?.qty_type === returnQuantityType
    ) {
      toast.error(
        `Quantity cannot exceed ${inquiryQty} ${alreadyData?.qty_type}`,
        {
          position: toast.POSITION.BOTTOM_RIGHT,
          autoClose: 1000,
        }
  ...
  const { displayCreditDebit, setDisplayCreditDebit } = useContext(
    DisplayCreditDebitContext
  );
  const [creditDebitData, setCreditDebitData] = useState(null);

  const handleOpenCreditDebit = () => {
    setCreditDebitData(alreadyData?.po_data?.[1]); // invoice data
    setDisplayCreditDebit(true);
  };
  ...
  return (
    <div className="w-full   overflow-x-hidden">
      <div className="flex flex-col p-2 pb-14">
          ...
          <div className="w-full h-full">
            <div className="grid 2xl:grid-cols-[3fr,1fr] xl:grid-cols-[2.7fr,1fr] lg:grid-cols-[2.3fr,1.3fr] gap-4 h-full">
                ...
                <div className="w-full">
                    ...
                    <div className="grid sm:grid-cols-[2fr,5fr,3.5fr] grid-cols-[1fr]">
                      ...
                      <div className="">
                        <h1 className="text-md font-semibold mb-2">
                          {alreadyData?.product?.name_of_chemical[0].toUpperCase() +
                            alreadyData?.product?.name_of_chemical.slice(1)}
                        </h1>

                        <div className="xl:flex block xl:mb-2">
                          <p className="text-slate-500 w-[220px] sm:text-sm text-xs">
                            CAS No: {alreadyData?.product?.CAS_number}
                          </p>
                          <p className="text-slate-500 sm:text-sm text-xs  ">
                            Category: {alreadyData?.category}
                          </p>
                        </div>
                        <div className="xl:flex block xl:mb-2">
                          {alreadyData.inq_type === "commercial" ? (
                            <p className="text-slate-500 sm:text-sm text-xs w-[220px]">
                              Quantity: {alreadyData?.inquiry_qty}
                              {alreadyData?.qty_type}
                            </p>
                          ) : (
                            <p className="text-slate-500 sm:text-sm text-xs w-[220px]">
                              One Lot Quantity: {alreadyData?.one_lot_qty}
                              {alreadyData?.one_lot_qty_type}
                            </p>
                          )}
                          <p className="text-slate-500 sm:text-sm text-xs">
                            Sub-Category:{" "}
                            {alreadyData?.subcategory
                              ? alreadyData.subcategory
                              : "-"}
                          </p>
                        </div>
                        ...
                        <div className="xl:flex block xl:mb-2">
                          <p className="text-slate-500 sm:text-sm text-xs  w-[220px]">
                            Country of Origin: {alreadyData?.country_origin}
                          </p>
                          <a
                            href={alreadyData?.COA}
                            target="_blank"
                            className="sm:text-sm text-xs text-blue-500 font-semibold underline"
                          >
                            COA
                          </a>
                        </div>
                        {alreadyData && alreadyData?.po_data?.length > 0 && (
                          <div className="flex flex-wrap gap-3 items-center">
                            <p className="text-slate-500 sm:text-sm text-xs font-semibold w-auto md:w-auto">
                              Generated Documents :
                            </p>
                            <p
                            ...
                            {alreadyData &&
                              alreadyData?.po_data?.[1]?.upload_COA && (
                                <p
                                  className="border-2 border-[#0A122A]/[.3] flex items-center gap-2 rounded-md py-1 px-2 text-[#0A122A99]/[.6] font-semibold sm:text-sm text-xs cursor-pointer hover:bg-darkBlue hover:text-white w-auto md:w-auto"
                            ...
                            {alreadyData &&
                              alreadyData?.po_data?.[1]?.upload_COA &&
                              (alreadyData?.is_returned ||
                                returnRequests.some((request) =>
                                  [
                                    "refund_successful",
                                    // ✅ ADD NEW REFUND STATUS OPTIONS
                                    "refund_processed",
                                    "credit_note_issued",
                                    "debit_note_issued",
                                    "amount_adjusted",
                      ...
                      <div className="flex flex-col justify-evenly">
                        <p className="font-medium sm:text-sm text-xs sm:text-right text-left">
                          Date : {formatDate(alreadyData?.createdAt)}
                        </p>
                        <p className="font-medium sm:text-sm text-xs sm:text-right text-left">
                          Inquiry Type :{" "}
                          {capitalizeFirstLetter(alreadyData?.inq_type)}
                        </p>
                        ...
                        {alreadyData?.inq_type === "commercial" && (
                          <>
                            {inquiryStatus !== "pending" &&
                              ...
                              inquiryStatus !== "approved" && (
                                <>
                                  {userPost === "buyer" && (
                                    <p
                                      className="font-medium text-blue-500 text-right underline cursor-pointer text-[14px]"
                                      onClick={() =>
                                        navigate("/company/delete-inquiries", {
                                          state: {
                                            productId:
                                              alreadyData?.product?._id,
                                            data: alreadyData?.inquiry_id,
                                          },
                                        })
                                      }
                                    >
                                      Delete Other Inquiries Of Same Chemical
                                    </p>
                                  )}

                                  {Number(finalPrice) ===
                                    Number(sumPrices(paymentHistory)) ? (
                                    <p className="font-medium sm:text-sm text-xs text-right">
                                      Payment Status :
                                      <span className="bg-green-500 text-white rounded-xl px-2 py-1 ms-2">
                                        Paid
                                      </span>
                                    </p>
                                  ) : (
                                    <p className="font-medium sm:text-sm text-xs text-right">
                                      ...
                                      {userPost !== "seller" && (
                                        <span
                                          className="text-white bg-blue-600 px-1 cursor-pointer"
                                          onClick={() =>
                                            setOpenPaymentByStatus(true)
                                          }
                                        >
                                          +
                                        </span>
                                      )}
                                      <span className="bg-red-500 text-white rounded-xl px-2 py-1">
                                        Pending
                                      </span>
                                    </p>
                                  )}
                            ...
                            {/* {inquiryStatus === "delivered" && userPost === "buyer" && (
                              <div className='flex items-center gap-2 ml-auto '>
                                <span className='font-medium sm:text-sm text-xs'>Return Status :</span>
                                <button
                                  className='bg-yellow-500 w-[75px] text-white rounded-xl px-4 py-1 text-sm font-medium hover:bg-yellow-600 transition cursor-pointer'
                            ...
                            {inquiryStatus === "delivered" &&
                              userPost === "buyer" &&
                              // ADD THESE CONDITIONS TO HIDE RETURN BUTTON
                              !alreadyData?.is_returned &&
                              !returnRequests.some((request) => {
                                const completedStatuses = [
                                  "rejected", // ✅ KEEP "rejected" IN THE LIST
                                  "refund_successful",
                  ...
                  <div className="rounded mb-10">
                    <div className="grid grid-cols-[0.8fr,5fr]">
                      {userPost === "buyer" && (
                        <div className="sm:block hidden">
                          {alreadyData?.seller_company?.seller_other_info
                            ?.length !== 0 &&
                            alreadyData?.seller_company?.seller_other_info?.[0]
                              ?.logo !== "" ? (
                            <a
                              href={`/company-profile/${alreadyData?.seller_company?._id}`}
                              target="_blank"
                              className="cursor-pointer"
                            >
                              <img
                                src={
                                  alreadyData?.seller_company
                                    ?.seller_other_info?.[0]?.logo
                                }
                                alt=""
                                className="mb-2 h-[80px] w-[80px] object-cover rounded-full"
                              />
                            </a>
                          ) : (
                            <a
                            ...
                            >
                              <div className="h-16 w-16 bg-gray-200 rounded-full flex items-center justify-center mb-2">
                                <p className="text-xl font-semibold text-gray-600">
                                  {alreadyData?.seller_company?.company_name
                                    .slice(0, 2)
                                    .toUpperCase()}
                                </p>
                              </div>
                      ...
                      {userPost === "seller" && (
                        <div className="sm:block hidden">
                          {alreadyData?.buyer_company?.buyer_other_info
                            ?.length !== 0 &&
                            alreadyData?.buyer_company?.buyer_other_info?.[0]
                              ?.logo !== "" ? (
                            <a
                              href={`/company-profile/${alreadyData?.buyer_company?._id}`}
                              target="_blank"
                              className="cursor-pointer"
                            >
                              <img
                                src={
                                  alreadyData?.buyer_company
                                    ?.buyer_other_info?.[0]?.logo
                                }
                                alt=""
                                className="mb-2 h-[80px] w-[80px] object-cover rounded-full"
                              />
                      ...
                      <div className="">
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <p className="sm:text-xl text-xs sm:block hidden font-semibold">
                            {(userPost === "seller"
                              ...
                              : alreadyData?.seller_company?.mode_of_business
                            )
                              ?.map((value) =>
                                value == "manufacture"
                                  ? "Manufacturer"
                        ...
                        <div className="sm:flex block items-center mb-2">
                          ...
                          <p className="text-slate-500 sm:text-sm text-xs sm:ps-3">
                            GST No:{" "}
                            {userPost === "seller"
                              ? alreadyData?.buyer_company?.gst
                              : alreadyData?.seller_company?.gst}
                          </p>
                        </div>
                        <div className="mt-3">
                          <p className="border-2 border-[#0A122A]/[.3] flex gap-2 rounded-md py-1 px-2 text-[#0A122A99]/[.6] font-semibold sm:text-sm text-xs">
                            <img src={location} alt="" className="h-[20px]" />{" "}
                            {userPost === "seller"
                              ? alreadyData?.buyer_company?.address
                              : alreadyData?.seller_company?.address}{" "}
                            {userPost === "seller"
                              ? alreadyData?.buyer_company?.city
                              : alreadyData?.seller_company?.city}
                            ,{" "}
                            {userPost === "seller"
                              ? alreadyData?.buyer_company?.state
                              : alreadyData?.seller_company?.state}{" "}
                            {userPost === "seller"
                              ? alreadyData?.buyer_company?.pincode
                              : alreadyData?.seller_company?.pincode}
                          </p>
                          <div className="flex flex-wrap gap-5 mt-4">
                            <p className="flex items-center gap-2 rounded-md py-1 px-2 border-2 border-[#0A122A]/[.3] text-[#0A122A99]/[.6] font-semibold sm:text-sm text-xs">
                              <img
                                src={telephone}
                                alt=""
                                className="sm:h-[17px] h-[14px]"
                              />
                              {userPost === "seller"
                                ? alreadyData?.buyer_company?.landline_num
                                : alreadyData?.seller_company?.landline_num}
                            </p>
                            <p className="flex items-center gap-2 rounded-md py-1 px-2 border-2 border-[#0A122A]/[.3] text-[#0A122A99]/[.6] font-semibold text-sm">
                              ...
                              />
                              {userPost === "seller"
                                ? alreadyData?.buyer_company?.mobile_num
                                : alreadyData?.seller_company?.mobile_num}
                            </p>
                            <p className="flex items-center gap-2 rounded-md py-1 ps-2 sm:pe-2 pe-5 border-2 border-[#0A122A]/[.3] text-[#0A122A99]/[.6] font-semibold sm:text-sm text-xs w-auto truncate max-w-[280px]">
                              <img
                                src={mail}
                                alt=""
                                className="sm:h-[14px] h-[11px]"
                              />
                              {userPost === "seller"
                                ? alreadyData?.buyer_company?.emailid
                                : alreadyData?.seller_company?.emailid}
                            </p>
                            <p className="flex items-center gap-2 rounded-md py-1 px-2 border-2 border-[#0A122A]/[.3] text-[#0A122A99]/[.6] font-semibold sm:text-sm text-xs">
                              <img src={location} alt="" className="h-[20px]" />
                              {userPost === "seller"
                              ...
                              ,{" "}
                              {userPost === "seller"
                                ? alreadyData?.buyer_company?.country
                                : alreadyData?.seller_company?.country}
                            </p>
                          </div>
                        </div>
                      </div>
                      ...
                      <>
                        ...
                        {isVisible && (
                          <div className="mb-10 border-2 border-gray-200 rounded-3xl px-5 py-5">
                            ...
                            <table
                              style={{ border: "none" }}
                              className="table-auto w-full border-0"
                            >
                              <tbody>
                                {paymentHistory &&
                    ...
                    <div className="overflow-x-auto sm:overflow-x-visible">
                      ...
                      >
                        ...
                        <div
                          ...
                          style={{
                            width: `${Math.min((userPost === "seller" ? percentCompleteForSeller : percentCompleteForBuyer) + 20, 100)}%`,
                    ...
                    >
                            ...
                            {status === "Negotiation" && (
                              <p className="text-[10px] font-medium absolute w-[90px] left-[-50%] bottom-[-30px] text-center">
                                {(visitDate &&
                                  formatDate(
                                    visitDate?.find(
                                      (visit) => visit.status === "Negotiation"
                                    )?.dateAndTime
                                  )) ||
                                  ""}
                              </p>
                      ...
                      <hr
                        ...
                        style={{
                          ...
                          background: `linear-gradient(to right, #09BB44 ${percentCompleteForBuyer + 20
                            }%, rgba(169, 169, 169, 0.5) ${percentCompleteForBuyer}%)`,
                        }}
                      />
                    </div>
                  )}
                  ...
                  {returnRequests.length > 0 && (
                    <div className="border border-[#0A122A]/0.1 mt-[100px] rounded-2xl shadow pt-5 pb-4 px-5 mb-5">
                      {returnRequests.map((request) => {
                        // Define all return statuses including new ones
                        ...
                        // Define basic flow statuses for progress bar (including rejected flow)
                        const BASIC_FLOW_STATUSES = [
                          "pending",
                          "under_review",
                          "approved",
                          "dispatch",
                        ...
                        // Define rejected flow statuses
                        const REJECTED_FLOW_STATUSES = [
                          "pending",
                          "under_review",
                          "rejected",
                        ];

                        // Check if current status is in rejected flow
                        const isRejectedFlow = REJECTED_FLOW_STATUSES.includes(
                          request.status
                        );

                        // Use appropriate flow based on current status
                        const currentFlowStatuses = isRejectedFlow
                          ? REJECTED_FLOW_STATUSES
                          : BASIC_FLOW_STATUSES;

                        // Find current status index for current flow
                        const currentReturnStatusIndex =
                          currentFlowStatuses.indexOf(request.status);
                        const isCompletedStatus = ALL_RETURN_STATUSES.slice(
                          6
                        ).includes(request.status);
                                ...
                                <button
                                  onClick={() => {
                                    if (selectedReturnRequest === request._id) {
                                      ...
                                      fetchReturnStatusHistory(request._id);
                                    }
                                  }}
              ...
              <div>
                  ...
                  <div className="border border-[#0A122A]/0.1 rounded-2xl shadow pt-5 pb-4  px-5 mb-5">
                    <div className="flex justify-between mb-2">
                      ...
                      <p className="text-sm font-medium">
                        {alreadyData?.inquiry_qty}
                        {alreadyData?.qty_type}
                      </p>
                    </div>
                    <div className="flex justify-between mb-2">
                      <p className="text-[#0A122A] text-sm">Payment Term :</p>
                      <p className="text-sm text-[#0A122A] font-medium">
                        {alreadyData?.payment_terms}
                      </p>
                    </div>
                    <div className="flex justify-between mb-2">
                      <p className="text-[#0A122A] text-sm">Delivery Time :</p>
                      <p className="text-sm text-[#0A122A] font-medium">
                        {alreadyData?.delivery_time}
                      </p>
                    </div>
                    <div className="flex justify-between mb-2 ">
                      <p className="text-[#0A122A] text-sm w-[140px]">
                        Inco Terms :
                      </p>
                      <p className="text-sm text-[#0A122A] font-medium text-end">
                        {alreadyData?.inco_terms}
                      </p>
                    </div>
                    <div className="flex justify-between items-center">
                      <p className="text-[#0A122A] text-sm font-semibold">
                        Inquiry Status :
                      </p>
                      <div className="relative">
                        ...
                        {dropdownStates &&
                          ...
                          inquiryStatus !== "in transit" && (
                            ...
                            >
                              <div className="py-3">
                                <h1 className="text-sm font-semibold px-3 mb-2">
                                  Change status
                                </h1>
                          ...
                          {userPost === "buyer" ? (
                            <>
                              {inquiryStatus === "po" ? (
                                ...
                                <>
                                  ...
                                  {deliveredForm && (
                                    <div>
                                      <div className="flex items-center gap-3 mt-3">
                                        <p className="w-[70px] font-semibold text-lg">
                                          {randomNumbers.num1} +{" "}
                                          {randomNumbers.num2} =
                                        </p>
                                        <input
                                        ...
                                        <button
                                          ...
                                          className="w-[80px] text-white rounded-[10px] bg-darkBlue font-semibold text-sm px-1 py-2"
                                        >
                                          OK
                                        </button>
                                      </div>
                                  ...
                                  <button
                                    className="w-full bg-black text-white rounded-lg py-2 text-sm font-semibold mt-4"
                                ...
                                <button
                                  className="w-full bg-black text-white rounded-lg py-2 text-sm font-semibold"
                            ...
                            <>
                              {inquiryStatus === "po" ? (
                                <button
                                  className="w-full bg-black text-white rounded-lg py-2 text-sm font-semibold"
                                  onClick={() => setGenrateInvoice(true)}
                                >
                                  Generate Invoice
                                </button>
                              ) : inquiryStatus === "invoice" ? (
                  ...
                  <>
                    <div className="border border-[#0A122A]/0.1 rounded-2xl shadow pt-5 pb-4  px-5 mb-5">
                      <div className="flex justify-between mb-3">
                        ...
                        <p className="text-sm font-medium">
                          {formatDate(alreadyData?.createdAt)}
                        </p>
                      </div>
                      <div className="flex justify-between mb-3">
                        <p className="text-[#0A122A] text-sm">Inquiry Type :</p>
                        <p className="text-sm text-[#0A122A] font-medium">
                          {alreadyData?.inq_type}
                        </p>
                      </div>
                      <div className="flex justify-between mb-3">
                        <p className="text-[#0A122A] text-sm">Quantity :</p>
                        <p className="text-sm text-[#0A122A] font-medium">
                          {alreadyData?.total_lot * alreadyData?.one_lot_qty}
                          {alreadyData?.one_lot_qty_type}
                        </p>
                      </div>
                      <div className="flex justify-between mb-3">
                              ...
                              <>
                                {inquiryStatus === "po" ? (
                                  <>
                                    ...
                                    <button
                                      className="mt-3 w-full bg-black text-white rounded-lg py-2 text-sm font-semibold"
                                      onClick={handlePaymentOfSampleInquiry}
                                    >
                                      Payment
                                    </button>
                                  </>
                ...
                {/* Move this return requests section outside the product details area */}
                {returnRequests.length > 0 && (
                  <div className="border border-[#0A122A]/0.1 rounded-2xl shadow pt-5 pb-4 px-5 mb-5 mt-5">
                    <h1 className="text-xl font-medium mb-2 border-b-2 ">
                      Return Requests
                    </h1>
                    {returnRequests.map((request) => (
                      <div key={request._id} className="mb-4 p-3 rounded-lg">
                        <div className="flex justify-between items-start mb-2">
                          <div>
                            <p className="text-sm font-medium">
                          ...
                          <span
                            className={`px-2 py-1 rounded text-xs font-medium whitespace-nowrap ${getStatusColorClass(
                              request.status
                            )}`}
                          >
                            {request.status
                        ...
                        <div className="flex flex-wrap gap-2 mt-3">
                          ...
                          {userPost === "seller" && (
                            <>
                              ...
                              {/* Seller: under_review → approved OR rejected */}
                          ...
                          {/* FINAL STATUS - No actions for anyone */}
                          {(request.status === "rejected" ||
                ...
                {finalPaymentDetails && location.state !== "returnOrder" && (
                  <div className="border border-[#0A122A]/0.1 rounded-2xl shadow pt-3 pb-7 mt-5 px-5 mb-5">
                    ...
                    <div className="grid grid-cols-[1.4fr,2fr] mb-2">
                      ...
                      <p className="text-sm flex items-center justify-end text-end">
                        {finalPaymentDetails?.quantity}
                        {finalPaymentDetails?.quantity_type}
                      </p>
                    </div>
                    <div className="grid grid-cols-[1.4fr,2fr] mb-2">
                      <div>
                      ...
                      <p className="text-sm flex items-center justify-end text-end">
                        <RupeesIcon /> {finalPaymentDetails?.final_price}
                      </p>
                    </div>
                    <div className="grid grid-cols-[1.4fr,2fr] mb-2">
                      <div>
                      ...
                      <p className="text-sm flex items-center justify-end text-end">
                        {finalPaymentDetails?.payment_terms}
                      </p>
                    </div>
                    <div className="grid grid-cols-[1.4fr,2fr] mb-2">
                      <div>
                      ...
                      <p className="text-sm flex items-center justify-end text-end">
                        {finalPaymentDetails?.delivery_time}
                      </p>
                    </div>
                    <div className="grid grid-cols-[1.4fr,2fr] mb-2">
                      <div>
                      ...
                      <p className="text-sm flex items-center justify-end text-end">
                        {finalPaymentDetails?.inco_terms}
                      </p>
                    </div>
                    ...
                    {alreadyData?.inq_type === "commercial" ? (
                      <>
                        <div className="border border-[#0A122A]/0.1 rounded-2xl shadow pt-3 pb-7  px-5">
                          ...
                          <div className="flex justify-between mb-2">
                            <div>
                              <p className="text-[#0A122A] text-sm">
                                Approx Price
                              </p>
                            </div>
                            {sendRequestCancel ? (
                              ...
                              <div>
                                ...
                                <p className="text-sm flex items-center justify-end">
                                  <RupeesIcon />
                                  {formatPrice(alreadyData?.max_price)}
                                </p>
                              </div>
                            )}
                          </div>
                          <div className="flex justify-between mb-2">
                            <p className="text-[#0A122A] text-sm">Discount</p>
                            <p className="text-sm text-[#FA3434] flex items-center">
                              -<RupeesIcon /> 0.0
                            </p>
                          </div>
                          <div className="flex justify-between mb-2">
                            <p className="text-[#0A122A] text-sm">Tax</p>
                            <p className="text-sm text-[#00B517] flex items-center">
                              +<RupeesIcon /> 0.0
                            </p>
                          </div>
                          ...
                          <div className="flex justify-between">
                            ...
                            <div>
                              ...
                              <p className="text-sm flex items-center font-medium justify-center">
                                -
                              </p>
                              <p className="text-sm flex items-center font-medium justify-end">
                                <RupeesIcon />
                      ...
                      <div className="border border-[#0A122A]/0.1 rounded-2xl shadow pt-5 pb-4  px-5 mb-5">
                        <div className="flex justify-between mb-2">
                          <div>
                            <p className="text-[#0A122A] text-sm w-full">
                              Subtotal :
                            </p>
                          </div>
                          <p className="text-sm font-medium flex items-center">
                            <RupeesIcon />{" "}
                            {alreadyData?.total_lot *
                              alreadyData?.one_lot_qty_price}
                          </p>
                        </div>
                        <div className="flex justify-between mb-2">
                          <p className="text-[#0A122A] text-sm">Discount :</p>
                          <p className="text-sm text-[#0A122A] font-medium text-red-500 flex items-center">
                            - <RupeesIcon /> 0.0
                          </p>
                        </div>
                        <div className="flex justify-between mb-2">
                          <p className="text-[#0A122A] text-sm">Tax :</p>
                          <p className="text-sm text-[#0A122A] font-medium text-green-500 flex items-center">
                            + <RupeesIcon /> 0.0
                          </p>
                        </div>
                        <div className="flex justify-between mb-2 ">
                          ...
                          <p className="text-sm text-[#0A122A] font-medium text-end text-green-500 flex items-center">
                            + <RupeesIcon /> 0.0
                          </p>
                        </div>
                        <hr />
                        <div className="flex justify-between mt-3">
                          <p className="text-[#0A122A] text-lg w-[160px] font-bold">
                            Total :
                          </p>
                          <div>
                            <p className="text-sm flex items-center justify-end">
                  ...
                  <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-md py-4">
                    <div class="bg-white pt-2">
                      ...
                      <div class="bg-white pt-3 flex justify-end gap-3 px-5">
                        <button
                          onClick={() => setGenrateInvoice(false)}
                          type="button"
                          class="inline-flex w-[100px] justify-center rounded-md bg-white px-3 py-2 text-sm font-medium  shadow-sm ring-1 ring-inset ring-gray-300 sm:mt-0"
                        >
                          Cancel
                        </button>
                        <button
                          ...
                          type="button"
                          class="inline-flex w-[70px] justify-center rounded-md bg-darkBlue py-2 text-sm font-medium text-white shadow-sm ring-1 ring-inset ring-gray-300 sm:mt-0"
                        >
                          Submit
                        </button>
                      </div>
                ...
                <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-2xl">
                  ...
                  <div class="bg-white flex justify-end mx-8 py-3">
                    <button
                      ...
                      onClick={() => setOpenPaymentByStatus(false)}
                    >
                      Cancel
                    </button>
                    <button
                      type="button"
                      class="inline-flex w-full justify-center rounded-md bg-[#0A122A] px-3 py-2 text-sm font-semibold text-white shadow-sm sm:w-auto ml-4"
                      ...
                      <div className="grid grid sm:grid-cols-[1fr,1fr,1fr,1fr] grid-cols-[1fr,1fr] gap-4 mb-5">
                        ...
                        <div className="w-full">
                          ...
                          <select
                            className="w-full bg-transparent border-2 text-slate-500 rounded text-sm py-1 px-3 w-full"
                            onChange={(e) =>
                              setUpdatedQuantityType(e.target.value)
                            }
                          >
                        ...
                        <div className="w-full">
                          <p className="mb-1 text-xs font-medium">
                            Final Price Per {updatedQuantityType}
                          </p>
                          <input
                            type="text"
                      ...
                      <div className="grid grid sm:grid-cols-[1fr,1fr,1fr] grid-cols-[1fr,1fr] gap-4 mb-5">
                        <div className="w-full">
                          ...
                          <select
                            className="w-full bg-transparent border-2 text-slate-500 rounded text-sm py-1 px-3 w-full"
                            onChange={(e) =>
                              setUpdatedPaymentTerm(e.target.value)
                            }
                          >
                            <option value="" selected disabled>
                              Select Payment Term
                            ...
                            <option value="15 Days Credit">
                              Credit (15 Days)
                            </option>
                            <option value="30 Days Credit">
                              Credit (30 Days)
                            </option>
                            <option value="45 Days Credit">
                              Credit (45 Days)
                            </option>
                            <option value="60 Days Credit">
                              Credit (60 Days)
                            </option>
                            <option value="90 Days Credit">
                              Credit (90 Days)
                            </option>
                          </select>
                        </div>
                        <div className="w-full">
                          ...
                          <select
                            className="w-full bg-transparent border-2 text-slate-500 rounded text-sm py-1 px-3 w-full"
                            onChange={(e) =>
                              setUpdatedDeliveryTime(e.target.value)
                            }
                          >
                            <option value="" disabled selected>
                            ...
                            <option value="Immediate">Immediate</option>
                            <option value="Delivery in 15 Days">
                              Delivery in 15 Days
                            </option>
                            <option value="Delivery in 30 Days">
                              Delivery in 30 Days
                            </option>
                          </select>
                        </div>
                        <div className="w-full">
                          ...
                          <select
                            className="w-full bg-transparent border-2 text-slate-500 rounded text-sm py-1 px-3 w-full"
                            onChange={(e) =>
                              setUpdatedIncoTerms(e.target.value)
                            }
                          >
                            <option value="" disabled selected>
                              Inco Terms
                            ...
                            <option value="CIF - Cost, insurance & Fright">
        ...
        {successGenratePo && (
          <div
            className="fixed inset-0 z-10 flex justify-center items-center bg-gray-500 bg-opacity-75"
            ...
            <div className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl max-w-md py-4">
              <div className="bg-white py-3 w-[400px]">
                <div className="flex flex-col items-center">
                  ...
                  <h3 className="font-semibold text-xl mt-4 text-center">
              ...
              <div className="bg-white mx-8 pb-3">
                <button
                  onClick={successGeneratePOclose}
                  type="button"
                  className="mt-3 inline-flex w-full justify-center rounded-md bg-darkBlue px-3 py-2 text-sm font-medium text-white shadow-sm ring-1 ring-inset ring-gray-300 sm:mt-0 sm:w-[100%]"
                >
                  Okay
                </button>
              </div>
        ...
        {isOpenPhoto && (
          <div
            className="fixed inset-0 z-10 flex justify-center items-center bg-black bg-opacity-30"
              ...
              <div className="py-3">
                <div className="">
                  <div className="flex justify-center gap-3 mb-2">
                    <button className="bg-darkBlue text-sm text-white px-5 py-1 rounded-[15px]">
                      Edit PO
                    </button>
                    <button
                      onClick={() => setSuccessGenratePo(true)}
                      className="bg-darkBlue text-sm text-white px-5 py-1 rounded-[15px]"
                    >
                      Generate PO
                    </button>
                  </div>
                ...
                <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-md py-4">
                  <div class="bg-white py-3">
                    <div class="flex flex-col items-center">
                      <img
                        src={cross}
                        alt=""
                        className="h-[50px] bg-gray-200 px-3 py-3 rounded-full"
                      />
                      <h3 className="font-semibold sm:text-2xl text-lg sm:text-left text-center  mt-4">
                  ...
                  <div class="bg-white mx-8 py-3">
                    <button
                      onClick={handleYes}
                      type="button"
                      class="sm:mt-3 inline-flex w-full justify-center rounded-md bg-white px-3 py-3 text-sm font-medium  shadow-sm ring-1 ring-inset ring-gray-300 sm:mt-0 sm:w-[100%] mb-3"
                    >
                      Yes
                    </button>
                    <button
                      onClick={cancelNo}
                      type="button"
                      class="sm:mt-3 inline-flex w-full justify-center rounded-md bg-darkBlue px-3 py-3 text-sm font-medium text-white shadow-sm ring-1 ring-inset ring-gray-300 sm:mt-0 sm:w-[100%]"
                    >
                      No
                    </button>
                  </div>
                    ...
                    <div class="bg-white pt-2">
                      ...
                      <div className="bg-gray-100 px-5 pt-3 pb-2">
                        <textarea
                          ...
                          id=""
                          rows="3"
                          className="bg-transparent w-full outline-none border-2 border-gray-200 rounded-md ps-3 pt-1 placeholder:text-sm"
                          placeholder="Give reason of canceling"
                        ></textarea>
                      </div>
                      <div class="bg-white pt-3 flex justify-end gap-3 mt-3 px-5">
                        ...
                        <button
                          ...
                          type="button"
                          class="inline-flex w-[100px] justify-center rounded-md bg-darkBlue px-3 py-2 text-sm font-medium text-white shadow-sm ring-1 ring-inset ring-gray-300 sm:mt-0"
                        >
                          Submit
                        </button>
                      </div>
              ...
              <div className="bg-white py-3 w-[400px]">
                <div className="flex flex-col items-center">
                  ...
                  <p className="mt-3 text-sm">
                    Your payment details was updated successfully
                  </p>
                </div>
              </div>
              <div className="bg-white mx-8 pb-3">
            ...
            <div className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl max-w-md py-4">
              <div className="bg-white py-3 w-[400px]">
                <div className="flex flex-col items-center">
                  ...
                  <p className="mt-3 text-sm">
                    Your Cancel Reason wan Submitted Successfully
                  </p>
                </div>
              </div>
              <div className="bg-white mx-8 pb-3">
        ...
        {deliveredSuccessfully && (
          <div
            className="fixed inset-0 z-10 flex justify-center items-center bg-darkBlue bg-opacity-35"
            onClick={successClose}
          >
            <div className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl max-w-md py-4">
              <div className="bg-white py-3 w-[400px]">
                        ...
                        <div className="h-full overflow-hidden">
                          <div className="sm:flex block gap-3 items-center pb-4">
                                  ...
                                  <>
                                    ...
                                    >
                                      <img
                                        ...
                                        alt=""
                                        className="sm:h-[60px] sm:w-[60px] h-[40px] w-[40px] object-cover rounded-full"
                                      />
                                    </a>
                                  </>
                                ) : (
                            ...
                            <div>
                              <p
                                ...
                                onClick={() => setMessageBoxOpne(false)}
                              >
                                {userPost === "buyer"
                                  ? alreadyData?.seller_company?.company_name
                                  : alreadyData?.buyer_company?.company_name}
                            ...
                            <div className="h-full">
                              <div
                                className="sm:flex hidden items-start rounded-lg cursor-pointer"
                            ...
                            {inquiryStatus !== "rejected" &&
                              ...
                              inquiryStatus !== "in transit" &&
                              inquiryStatus !== "delivered" &&
                              finalPaymentDetails?.request_status !==
                              "pending" && (
                                <p
                                  className="ms-auto underline text-blue-600 text-md font-semibold cursor-pointer"
                                  onClick={handlePopUp}
                                >
                                  Negotiation
                                </p>
                            ...
                            <FontAwesomeIcon
                              icon={faAngleDown}
                              className="ms-auto text-xl cursor-pointer sm:block hidden"
                              onClick={() => setMessageBoxOpne(false)}
                            />
                            <FontAwesomeIcon
                              icon={faAngleDown}
                              className="ms-auto text-xl cursor-pointer sm:hidden block  "
                              onClick={() => setMessageBoxOpne(false)}
                            />
                          </div>
                          <hr />
                          <div id="chat-container" className="h-full relative">
                            <div className="mt-5 flex flex-col overflow-scroll h-full pb-[40%]">
                              {messageDisplay &&
                                messageDisplay.map((e) => {
                                  const isCompanyMessage =
                                    e.senderId === CompanyId;
                                  const messageClass = isCompanyMessage
                                    ? "text-end"
                                    : "text-start";
                                  const bubbleClass = isCompanyMessage
                                    ? "bg-sky-950 text-white"
                                    : "bg-gray-100";
                                  const alignClass = isCompanyMessage
                                    ? "justify-end"
                                    : "justify-start";
                                  const timeAlignClass = isCompanyMessage
                                    ? "text-end"
                                    : "text-start";
                                  ...
                                  return (
                                    <div
                                      key={e._id}
                                      className={`mb-4 flex ${alignClass}`}
                                    >
                                      {e.quantity > 0 ? (
                                        isCompanyMessage ? (
                                          <div className="bg-gray-200 rounded-2xl px-5 py-4 mb-5">
                                            ...
                                            <p className="text-end text-sm font-medium mb-1">
                                              Request from you
                                            </p>
                                            <div className="border-t border-black"></div>
                                            <p className="text-end font-medium text-[10px] text-gray-500 mt-1 mb-3">
                                              {e?.datetime?.slice(11, 16)}
                                            </p>
                                            <p className="text-end text-sm font-medium mb-2">
                                              Quantity : {e.quantity}
                                              {e.quantity_type}
                                            </p>
                                            ...
                                            <p className="text-end text-sm font-medium mb-2">
                                              Payment Term : {e.payment_terms}
                                            </p>
                                            <p className="text-end text-sm font-medium mb-2">
                                              Delivery time : {e.delivery_time}
                                            </p>
                                            <p className="text-end text-sm font-medium mb-2">
                                              Inco Terms : {e.inco_terms}
                                            </p>
                                            {e?.request_status === "denied" ? (
                                              <button className="w-full bg-gray-400 rounded-[30px] py-[6px] text-sm mt-4">
                                                Denied
                                              </button>
                                            ) : e?.request_status ===
                                              "approved" ? (
                                              <button
                                                className="w-full border border-black bg-darkBlue text-white rounded-[30px] py-[6px] text-sm mt-4"
                                                onClick={() => {
                                                  handleCancelRequest(
                                                    "approved",
                                                    e._id
                                              ...
                                              "cancel" ? (
                                              <>
                                                <button className="w-full rounded-[30px] py-[6px] text-sm mt-4 bg-gray-400">
                                                  Canceled
                                                </button>
                                              </>
                                            ) : (
                                              <>
                                                {loader ? (
                                                  <button
                                                    type="button"
                                                    class="w-full rounded-[30px] py-[6px] text-sm mt-4 bg-gray-400"
                                                  >
                                                    <img
                                                      src={loaderImage}
                                                      alt=""
                                                  ...
                                                  <button
                                                    ...
                                                    className="w-full border border-black rounded-[30px] py-[6px] text-sm mt-4"
                                                  >
                                                    Cancel
                                                  </button>
                                                )}
                                          ...
                                          <div className="flex pb-[140px]">
                                            <div className="bg-gray-200 rounded-2xl px-5 py-4">
                                              ...
                                              <p className="font-medium text-[10px] text-gray-500 mt-1 mb-3">
                                                {e?.datetime?.slice(11, 16)}
                                              </p>
                                              ...
                                              {e?.request_status ===
                                                ...
                                                "approved" ? (
                                                <button className="w-full border border-black bg-darkBlue text-white rounded-[30px] py-[6px] text-sm mt-4">
                                                  Approved
                                                </button>
                                              ) : e?.request_status ===
                                                "cancel" ? (
                                        ...
                                        <div
                                          className={`${bubbleClass} px-5 py-2 rounded-xl`}
                                        >
                                          <p
                                            className={`${messageClass} text-sm`}
                                          >
                                            {e.message}
                                          </p>
                                          <p
                                            className={`text-xs mt-3 text-gray-400 ${timeAlignClass}`}
                                          >
                                            {e?.datetime?.slice(11, 16)}
                                          </p>
                                        </div>
                          ...
                          <div className="absolute bottom-0 w-[94%] bg-white py-5 flex gap-2">
                            ...
                            <FontAwesomeIcon
                              icon={faFileImage}
                              className="absolute top-[38%] ms-5 text-lg"
                            />
                          </div>
                        </div>
                      </div>
                ...
                <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-md py-4">
                  ...
                  <div class="bg-white mx-10 py-3">
                    <button
                      onClick={approveYes}
                      type="button"
                      class="mt-3 inline-flex w-full justify-center rounded-md bg-white px-3 py-2 text-sm font-medium  shadow-sm ring-1 ring-inset ring-gray-300 sm:mt-0 sm:w-[100%] mb-3"
                    >
                      Yes
                    </button>
                    <button
                      onClick={approveNo}
                      type="button"
                      class="mt-2 inline-flex w-full justify-center rounded-md bg-darkBlue px-3 py-2 text-sm font-medium text-white shadow-sm ring-1 ring-inset ring-gray-300 sm:mt-0 sm:w-[100%]"
                    >
                      No
                    </button>
                  </div>
                ...
                <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-md py-4">
                  <div class="bg-white py-3">
                    <div class="flex flex-col items-center">
                      ...
                      <h3 className="font-semibold text-xl mt-4 mx-8 text-center">
                  ...
                  <div class="bg-white mx-10 py-3">
                    ...
                    <button
                      onClick={() => setStatusDispatch(false)}
                      type="button"
                      class="mt-2 inline-flex w-full justify-center rounded-md bg-darkBlue px-3 py-2 text-sm font-medium text-white shadow-sm ring-1 ring-inset ring-gray-300 sm:mt-0 sm:w-[100%]"
                    >
                      No
                ...
                <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-md py-4">
                  <div class="bg-white py-3">
                    <div class="flex flex-col items-center">
                      <img src={right} alt="" className="h-[70px] " />
                      <h3 className="font-semibold text-xl mt-4">
                        Are You Sure You Want To In Trasit Order?
                      </h3>
                    </div>
                  </div>
                  <div class="bg-white mx-10 py-3">
                    ...
                    <button
                      onClick={() => setStatusTransit(false)}
                      type="button"
                      class="mt-2 inline-flex w-full justify-center rounded-md bg-darkBlue px-3 py-2 text-sm font-medium text-white shadow-sm ring-1 ring-inset ring-gray-300 sm:mt-0 sm:w-[100%]"
                    >
                      No
        ...
        {inquiryStatus !== "cancel" &&
          ...
          inquiryStatus !== "pending" && (
            <>
              {alreadyData?.inq_type === "commercial" && (
                <div
                  className="fixed bottom-[3%] right-[3%] xl:w-[30%] sm:w-[70%] w-[70%] cursor-pointer"
                  onClick={() => setMessageBoxOpne(true)}
                >
                    ...
                    <div className="h-full w-full border border-[#0A122A]/0.1 rounded-2xl shadow py-3 px-4">
                      {userPost === "buyer" && (
                        <div className="flex gap-3  items-center">
                          ...
                          <FontAwesomeIcon
                            icon={faAngleUp}
                            className="ms-auto text-xl cursor-pointer"
                          />
                        </div>
                      )}
                      {userPost === "seller" && (
                ...
                <div className="">
                  {sendData?.design === "design1" && (
                    <Invoice1
                      ...
                      title={
                        userPost === "buyer" ? "CREDIT NOTE" : "DEBIT NOTE"
                      }
                    />
                  )}
        ...
        {displayCreditDebit && creditDebitData && (
          <div
            className="fixed inset-0 z-10 flex justify-center items-center bg-darkBlue bg-opacity-35"
            onClick={successClose}
          >
            {/* <div className="fixed inset-0 bg-black bg-opacity-30 z-50 flex justify-center items-start overflow-y-auto py-10"> */}
            <div className="bg-white rounded-lg shadow-lg w-[60vw] max-h-[90vh] overflow-y-auto">
              <CreditDebitNote
                  ...
                  <div class="bg-white py-3">
                    <div class="flex flex-col items-center">
                      ...
                      <h3 className="font-semibold text-xl text-center px-5 mt-4">
                        With Your Current Membership Plan You Can Add Only{" "}
