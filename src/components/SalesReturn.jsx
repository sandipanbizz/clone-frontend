...
//         const res = await fetch(`${BASE_URL}api/return_order/getReturnRequests`, {
//             method: "GET",
//             headers: {
...
const Salesreturn = () => {
    ...
    const fetchPaymentHistory = async () => {
        try {
            const token = `Bearer ${localStorage.getItem("chemicalToken")}`;
            const res = await fetch(`${BASE_URL}api/return_order/getReturnRequests/sales`, {
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: token,
            ...
            const data = await res.json();
            console.log("Fetched sales return data:", data); // Debug log
            setList(data.data || []);
        } catch (error) {
    ...
    // Filter data with CORRECT data structure from your API
    const filteredData = list.filter(item => {
        // CORRECT data access based on your actual API response
        const chemicalName = item?.products?.[0]?.chem_name || '';
        const buyerName = item?.buyer_id?.company_name || '';
        ...
        const matchesBuyerName = String(buyerName).toLowerCase().includes(String(buyerQuery).toLowerCase());
                ...
                <div className="overflow-x-auto max-h-screen mt-4">
                    {sortedData.length === 0 ? (
                        <div className="flex justify-center items-center h-32">
                            <p className="text-gray-500">No sales return data found</p>
                        </div>
                    ) : (
                                    ...
                                    <React.Fragment key={item._id || index}>
                                        <tr className={`flex py-2 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-100'}`}>
                                            ...
                                            <td className="py-2 flex flex-col w-[150px]">
                                                ...
                                                <p className='font-semibold text-[14px]'>
                                                    {item?.buyer_id?.company_name || 'N/A'}
                                                </p>
                                            </td>
