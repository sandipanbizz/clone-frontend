...
const BuyingInquiry = () => {
  ...
  const fetchSellingInquiryList = async () => {
    ...
    setBuyingInquiryList(data.data);
  ...
  const filteredInquiryList = buyingInquiryList.filter((item) => {
    ...
    if (activeLink === 'commercial') return item.inq_type === 'commercial';
    return false;
  });
      ...
      <div className="flex flex-col p-2">
        ...
        <div className='flex flex-wrap gap-4 mt-7 items-center'>
          <div className="hidden sm:flex flex-wrap gap-7">
            ...
            <span
              className={`cursor-pointer pb-2 ${activeLink === 'accepted' && 'border-b-2 border-black'}`}
            ...
            <span
              className={`cursor-pointer pb-2 ${activeLink === 'commercial' && 'border-b-2 border-black'}`}
              ...
              {showDatePicker && (
                <div className='absolute shadow-lg rounded px-5 py-5 z-[11111111] bg-white left-[-100%] top-[100%]'>
                  <div className='mb-2'>
                    ...
                    <input type="date" className="text-sm border border-gray-300 rounded-md px-3 focus:outline-none focus:ring-0" value={startDate} onChange={handleStartDateChange} />
                  </div>
                  <div className='mb-2'>
                    <p className='text-xs mb-1 font-semibold'>End Date</p>
                    <input type="date" className="text-sm border border-gray-300 rounded-md px-3 focus:outline-none focus:ring-0" value={endDate} onChange={handleEndDateChange} min={startDate} />
                  </div>
                  <div className='flex gap-2 justify-center mt-4'>
                    <button className='bg-gray-500 text-white rounded px-3 py-1 text-xs' onClick={handleClear}>Clear</button>
                    <button className='bg-blue-500 text-white rounded px-3 py-1 text-xs' onClick={handleSearch}>Search</button>
                  </div>
                </div>
              )}
            </div>
          ...
          {filteredList && [...filteredList].reverse()?.filter((e) => e?.product?.name_of_chemical?.toLowerCase()?.includes(searchInput?.toLowerCase()))?.map((item, index) => {
            ...
            return (
              <div key={index} className="border-[3px] border-gray-200 rounded-[25px] py-4 cursor-pointer" onClick={() => handleNavigate(item)}>
                ...
                <p className='text-xs text-end flex items-center ms-3 mt-2'><span className='font-medium'>Inquiry Date : &nbsp;</span> <span>{item?.createdAt?.slice(0, 10)}</span></p>
                      ...
                      <hr
                        ...
                        style={{
                          ...
                          background: item?.inquiry_status?.[0]?.inquiry_status_value.length < 2
                            ...
                            : item?.inquiry_status?.[0]?.inquiry_status_value?.some((status) => status?.status === "Delivered" || status?.status === "Rejected")
