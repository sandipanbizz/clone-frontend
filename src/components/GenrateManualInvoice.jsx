...
const GenerateManualInvoice = () => {
    ...
    const generatePo = async () => {
        ...
        if (formData?.upload_COA === null && !isProforma) {
        ...
        if (isProforma) {
            formDataToSend.append('invoice_type', "performa_invoice");
        } else {
            formDataToSend.append('invoice_type', "tax_invoice");
        }
        ...
        const convertToNumberFields = (data) => ({
            ...data,
            bill_to_pincode: Number(data.bill_to_pincode),  // Convert to Number
            bill_to_phone: data.bill_to_phone && Number(data.bill_to_phone),      // Convert to Number
            shipped_to_pincode: Number(data.shipped_to_pincode), // Convert to Number
            shipped_to_phone: data.shipped_to_phone && Number(data.shipped_to_phone),     // Convert to Number
            grand_total: Number(data.grand_total),          // Convert to Number
        });

        // Always convert the necessary fields, regardless of invoice type
        const filteredData = isProforma
            ? {
                ...
                shipped_to_gst_in: formData.bill_to_gst_in,
                ...
                bill_to_pincode: Number(formData?.bill_to_pincode),
                bill_to_phone: Number(formData?.bill_to_phone),
                shipped_to_pincode: Number(formData?.shipped_to_pincode),
                shipped_to_phone: Number(formData?.shipped_to_phone),
            }
            : formData;

        // Convert fields for both proforma and non-proforma cases
        const convertedData = convertToNumberFields(filteredData);

        appendFormData(convertedData);
        setLoading(true);
    ...
    const handleGst = (e) => {
        ...
        setGstAmount((rate * quantity) * selectedGstPercentage / 100)
    }
                            ...
                            <tr className=''>
                                <td className='border-blue-400 border-r-[3px] font-medium ps-5 pe-4'>
                                    <div className="grid grid-cols-[1fr,3fr,] items-center pb-2">
                                        <p className=''>Name</p>
                                        <p className='flex items-center gap-2'>:
                                            <input type='text' name='bill_to_name' value={formData?.bill_to_name} disabled onChange={handleChange} className='w-full border-2 border-gray-300 px-2 py-1 rounded-md' />
                                        </p>
                                    </div>
                                </td>
                            ...
                            <tr className=''>
                                <td className='border-blue-400 border-r-[3px] font-medium ps-5 pe-4 pb-2 '>
                                    <div className="grid grid-cols-[1fr,3fr,] items-center">
                                        ...
                                        <p className='flex items-center gap-2'>:
                                            <select name='bill_to_country' value={formData?.bill_to_country} onChange={handleChange} disabled className='w-full border-2 border-gray-300 w-[70%] px-2 py-1 rounded-md'>
                                                <option value="">select</option>
                            ...
                            <tr className=''>
                                <td className='border-blue-400 border-r-[3px] font-medium ps-5 pe-4 pb-2 '>
                                    <div className="grid grid-cols-[1fr,3fr,] items-center">
                                        <p className=''>State</p>
                                        <p className='flex items-center gap-2'>:
                                            <select name='bill_to_state' onChange={handleChange} disabled value={formData?.bill_to_state} className='w-full border-2 border-gray-300 w-[70%] px-2 py-1 rounded-md'>
                                                <option value="">select</option>
                                                {states && states.map((e) => (
                                                    <option value={e.name}>{e.name}</option>
                                                ))}
                            ...
                            <tr className=''>
                                <td className='border-blue-400 border-r-[3px] font-medium ps-5 pe-4 pb-2 '>
                                    <div className="grid grid-cols-[1fr,3fr,] items-center">
                                        <p className=''>City</p>
                                        <p className='flex items-center gap-2'>:
                                            <select name='bill_to_city' value={formData?.bill_to_city} onChange={handleChange} disabled className='w-full border-2 border-gray-300 w-[70%] px-2 py-1 rounded-md'>
                                                <option value="">select</option>
                                                {cities && cities.map((e) => (
                                                    <option value={e.name}>{e.name}</option>
                                                ))}
