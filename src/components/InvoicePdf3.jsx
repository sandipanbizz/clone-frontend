...
const InvoicePdf3 = ({ data }) => {
    ...
    const [gstCheck, setGstCheck] = useState("sgst")
    ...
    return (
        <div className='bg-transparent flex flex-col items-center justify-center'>
            ...
            <div ref={divRef} className='bg-white px-10 py-10 w-[50vw] text-xs'>
                <div>
                    <h1 className='text-4xl font-bold text-[#0070C0] text-center mb-3'>{data?.invoice_type === "tax_invoice" ? "TAX INVOICE" : "PERFORMA INVOICE"}</h1>
                </div>
                <div className='border-[3px] border-blue-400'>
                    <div className='grid grid-cols-[1fr,1fr]'>
                        <div className='border-r-[3px] border-blue-400'>
                            <div className='flex py-2 px-4 gap-8 border-b-[3px] border-blue-400'>
                                <div>
                                ...
                                <div>
                                    <p className='font-semibold text-[14px]'>{data?.seller_company_details?.[0]?.company_name}</p>
                                    <p className='font-medium'>{data?.seller_company_details?.[0]?.address}</p>
                                    <p className='font-medium'>{data?.seller_company_details?.[0]?.city}, {data?.seller_company_details?.[0]?.state}, {data?.seller_company_details?.[0]?.country} - {data?.seller_company_details?.[0]?.pincode}</p>
                                    <p className='font-medium'>GSTIN : {data?.bill_to_gst_in}</p>
                                </div>
                            </div>
                            <div className='py-2 px-4 border-b-[3px] border-blue-400'>
                                <div>
                                    <p className='font-semibold text-sm text-gray-500'>Details of Buyer | Bill to :</p>
                                    <p className='font-semibold text-sm'>{data?.bill_to_name}</p>
                                    ...
                                    <div className='flex'>
                                        ...
                                        <p className='font-medium'>{data?.bill_to_gst_in}</p>
                                    </div>
                                    <p className='font-medium'>{data?.bill_to_phone}</p>
                                </div>
                            </div>
                            <div className='flex py-2 px-4 gap-8'>
                                <div>
                                    <p className='font-semibold text-sm text-gray-500'>Shipped to :</p>
                        ...
                        <div>
                            <div className='grid grid-cols-[1fr,1fr]'>
                                ...
                                <div className='border-b-[3px] border-r-[3px] border-blue-400 py-2 ps-5'>
                                    ...
                                    <p className='text-sm font-semibold'>{data?.invoice_no}</p>
                                </div>
                                <div className='border-b-[3px] border-blue-400 py-2 ps-5'>
                                    <p className='font-medium'>Invoice Date</p>
                                    <p className='text-sm font-semibold'>{formatDate(data?.invoice_date)}</p>
                                </div>
                                <div className='border-b-[3px] border-r-[3px] border-blue-400 py-2 ps-5'>
                                ...
                                {data?.eway_no !== "" && (
                                    <div className='border-b-[3px] border-r-[3px] border-blue-400 py-2 ps-5'>
                                        <p className='font-medium'>{inquiryType === "commercial" ? "E-Way No." : "EWB"}</p>
                                        <p className='text-sm font-semibold'>{data?.eway_no}</p>
                                    </div>
                                )}
                                {data?.due_date !== "" && (
                                    <div className='border-b-[3px] border-r-[3px] border-blue-400 py-2 ps-5'>
                                        <p className='font-medium'>Due Date</p>
                                        <p className='text-sm font-semibold'>{formatDate(data?.due_date)}</p>
                                    </div>
                                )}
                                {data?.packaging_type !== "" && (
                                    <>
                                        <div className=' py-2 ps-5'>
                                            <p className='font-medium'>Packaging Type</p>
                                            <p className='text-sm font-semibold'>{data?.packaging_type} </p>
                                            <p className='font-medium'>Total Unit</p>
                                            <p className='text-sm font-semibold'>{data?.packaging_no_of_bags} <br /> ({data?.packaging_weight}{data?.packaging_weight_type} / 1 {data?.packaging_type}) </p>
                                            {data?.mode_of_transport !== "" && (
                                                <p className='font-medium mt-2'>Mode Of Transport</p>
                                            )}
                                            {data?.mode_of_transport !== "" && (
                                                <p className='text-sm font-semibold'>{data?.mode_of_transport} </p>
                                            )}
                                        </div>
                                        <div className=' py-2 ps-5'>
                                            <p className='font-medium'>Vehicle Number</p>
                                            <p className='text-sm font-semibold'>{data?.vehicle_no} </p>
                                            {data?.delivery_time !== "" && (
                                                <p className='font-medium mt-2'>Deliverey Time</p>
                                            )}
                                            {data?.delivery_time !== "" && (
                                                <p className='text-sm font-semibold'>{data?.delivery_time} </p>
                                            )}
                                        </div>
                ...
                <table className="w-full table-auto mt-10">
                    ...
                    <tbody className=''>
                            ...
                            <tr className=''>
                                ...
                                <td className='border-blue-400 border-r-[3px] font-medium  py-2 text-center text-xs'>
                                    {gstCheck === "igst" ? (
                                        ...
                                        <span>{e.gstAmount / 2}({e.igst / 2}%)</span>
                                    )}
                                </td>
                                <td className='border-blue-400 border-r-[3px] font-medium  py-2 text-center text-xs'>
                                    <p className='flex items-center justify-center'>
                        ...
                        <tr className='border-blue-400 border-[3px] bg-blue-100 text-xs'>
                            ...
                            <td className='py-2 border-blue-400 border-r-[3px] text-end font-semibold pe-2'>{totals.totalQuantity.toFixed(2)}</td>
                ...
                <div className='border-blue-400 border-[3px] border-t-0 grid grid-cols-[1fr]'>
                    ...
                    <div className='grid grid-cols-[1fr,1fr] ms-3 py-3'>
                        <div>
                            <div className=''>
                                <div>
                                    <p className='font-semibold text-sm'>Bank Details</p>
                                    <div className='grid grid-cols-[1fr,2.5fr] mt-2'>
                                        <p className='text-gray-500'>Name :</p>
                                        <p className='font-semibold'>{data?.bank_details?.[0]?.bank_name}</p>
                                    </div>
                                    <div className='grid grid-cols-[1fr,2.5fr] mt-1'>
                    ...
                    <div className='grid grid-cols-[1fr,1fr] '>
                        <div className='flex flex-col  pb-3 ps-3 border-t-[3px] border-blue-400'>
                            <p className='text-lg font-semibold'>Terms and Conditions:</p>
                            <p className='text-gray-500 font-medium'>{data?.termsand_condition}</p>
                        </div>
                        <div className='border-blue-400 border-[3px] border-b-0 border-r-0'>
                            <div className='px-3 pt-2 h-full'>
                                ...
                                <div className='flex flex-col items-center h-full'>
                                    ...
                                    <div className='mt-auto mb-10 flex items-center gap-4'>
                                        <p className='font-semibold text-md'>Authorised Signatory</p>
